...
#cs
  ...
  **_WinAPI_PrivateExtractIcon
	...
	_WinAPI_RestartDlg
    _WinAPI_RestoreDC
	_WinAPI_ResumeProcess
	_WinAPI_ResumeThread
    _WinAPI_RGB
    _WinAPI_RoundRect
    _WinAPI_SaveDC
...
; ===============================================================================================================================
; _WinAPI_GetDateFormat()m, _WinAPI_GetDateFormat()
; ===============================================================================================================================
...
; Parameters.....: $hJob     - Handle to the job object to which the process will be associated.
...
Func _WinAPI_AssocGetPerceivedType($sExt)
	...
	Local $Ret = DllCall('shlwapi.dll', 'int', 'AssocGetPerceivedType', 'wstr', $sExt, 'int*', 0, 'dword*', 0, 'wstr*', '')
...
;===============================================================================
; Function Name:    _WinAPI_ChangeScreenResEx()
    ...
    If $B <> 0 Then
        ...
        Select
            ...
            Case $B = $DISP_CHANGE_SUCCESSFUL
                ...
                DllCall("user32.dll", "int", "SendMessage", "hwnd", $HWND_BROADCAST, "int", 0x007E, _
        ...
        EndSelect
    EndIf
EndFunc;==>_WinAPI_ChangeScreenResEx
...
Func _WinAPI_CreateStreamOnHGlobal($hGlobal = 0, $fDeleteOnRelease = True)
	...
	Local $Ret = DllCall('ole32.dll', 'int', 'CreateStreamOnHGlobal', 'ptr', $hGlobal, 'int', $fDeleteOnRelease, 'ptr*', 0)
...
;                  format梩he format cannot be started programmatically.
; Related........:
...
; #FUNCTION# ====================================================================================================================
; Name...........: _WinAPI_GetCommandLineFromPID
; Description....: 
; Syntax.........: _WinAPI_GetCommandLineFromPID ( $PID )
; Parameters.....: 
...
Func _WinAPI_GetFontResourceInfo($sFont, $fForce = 0)
	...
	If $fForce Then
		If Not _WinAPI_AddFontResourceEx($sFont) Then
			Return SetError(1, 0, '')
		EndIf
	EndIf
	...
	EndIf
	If $fForce Then
		_WinAPI_RemoveFontResourceEx($sFont)
	EndIf
	If Not IsArray($Ret) Then
		Return SetError(1, 0, '')
	EndIf
...
Func _WinAPI_GetHGlobalFromStream($hStream)
	...
	Local $Ret = DllCall('ole32.dll', 'int', 'GetHGlobalFromStream', 'ptr', $hStream, 'ptr*', 0)
...
; Return values..: Success  - The value specifies the requested information. If $iType is 0, return value may be one of the following values.
...
Func _WinAPI_GetProcessIoCounters($PID = 0)
	...
	Local $Ret = DllCall('kernel32.dll', 'int', 'GetProcessIoCounters', 'ptr', $hProc, 'ptr', DllStructGetPtr($tIO_COUNTERS))
...
; Description....: Retrieves an array of system information, or just one item based on parameter
; Syntax.........: _WinAPI_GetSystemInfo($iInformation=-1)
; Parameters.....: $iInformation = The piece of information you would like to collect, or -1 for an entire array:
;                            [1] = 处理器的体系结构 [0 = Intel, 6 = Itanium64, 9 = x64 (AMD or Intel), 0xFFFF = Unknown]
;                            [2] = 虚拟内存使用的页面文件大小
;                            [3] = 最小寻址空间
;                            [4] = 最大寻址空间
;                            [5] = 处理器掩码; 0..31 表示不同的处理器 - Set Bits indicate processor has been 'configured into the system' (MSDN)
;                                  [Bit 0 = processor # 0, Bit 31 = processor 31
;                            [6] = 处理器数目 [not the same as 'Logical Processors']
;                            [7] = 处理器类型 [386 = Intel 386, 486 = "" 486, 586 = Pentium, 2200 = Itanium64, 8664 = AMD x8664 (?)
;                            [8] = 虚拟内存空间的粒度Allocation Granularity ("granularity for the starting address at which virtual memory can be allocated" - MSDN)
;                            [9] = 处理器等级 (1 for Itanium64, otherwise for Processor Architecture 0 it represents CPU-vendor-specific info)
;                            [10] = 处理器版本 (no clue - check out http://msdn.microsoft.com/en-us/library/ms...58(VS.85).aspx)
;
; Return values..: Success - @error=0, and either a 10-element array filled in the order described above #1 at element [0], #10 at [9],
;                            or a # associated with $iInformation
;                  Failure - -1 return, @error is set:
;
;                              1 = invalid parameter (out of range)
;                              2 = DLL call error
;
; Author.........: Ascend4nt
; Modified.......:
...
Func _WinAPI_GetTcpTable($WSdll = "ws2_32.dll", $IHdll = "iphlpapi.dll")
...
Func _WinAPI_PrivateExtractIcon($sIcon, $iIndex, $iWidth, $iHeight)
	...
	If (@error) Or ($Ret[0] = 0) Or ($Ret[5] = 0) Then
		Return SetError(1, 0, 0)
	EndIf
	Return $Ret[5]
...
Func _WinAPI_ShellGetSpecialFolderLocation($CSIDL)
	...
	Local $Ret = DllCall('shell32.dll', 'int', 'SHGetSpecialFolderLocation', 'hwnd', 0, 'int', $CSIDL, 'ptr*', 0)
...
Func _WinAPI_ShellILCreateFromPath($sPath)
	...
	Local $Ret = DllCall('shell32.dll', 'int', 'SHILCreateFromPath', 'wstr', $sPath, 'ptr*', 0, 'dword*', 0)
...
;                  |FALSE    - ASCII.
; Return values..: Success   - The length of the string, in TCHAR values (not including the terminating null character).
;                  Failure   - 0 and sets the @error flag to non-zero.
; Author.........: Yashied
; Modified.......:
; Remarks........: None
...
; Example........: Yes
; ===============================================================================================================================
Func _WinAPI__ResumeProcess($hProcess)
     Local $Handle = DllCall("kernel32.dll", 'int', 'OpenProcess', 'int', 0x1f0fff, 'int', False, 'int', $hProcess)
     Local $Ret = DllCall("ntdll.dll","int","NtResumeProcess","int",$Handle[0])
     DllCall('kernel32.dll', 'ptr', 'CloseHandle', 'ptr', $Handle)
	If (@error) Or ($Ret[0] = 0) Then
